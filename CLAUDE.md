...
| V2.5.0 | 內容基礎建設（data-src 單一真相＋build/validate/bump/tests/make）|
| V2.6.0 | 熱門國地標 +3（delta 增量首役）|
| V2.7.0 | 首都冷知識 588（資料集增為9）|
| V2.7.1 | 全庫語句盤檢（改寫11條同義反覆＋validator中文語法註解）|
| V2.7.2 | 修返回手勢關閉App；world.js地圖防護網 |
| V2.8.0 | **三大功能**：(1) **常錯復習**——bumpMiss 擴及三層級（鍵 `c|名`/`w|名`/`d|縣市|名`），首頁「復習最常錯」卡片可點，`startReview()` 取錯最多的層級組 map2name 復習；(2) **分區小知識**——新資料集 `district-facts.json`（六優先縣市台中/台南/彰化/台北/桃園/高雄共 155 區 ×3＝465 段，key=縣市/區），分區題答完顯示，validate 對照 DISTRICTS 交叉比對；(3) **冷門國擴庫**——163 非熱門國 8→**12** 條（世界小知識 1802→**2454**），下限鎖 12。delta 增量＋主檔補足混用，撞重複整批中止機制實測觸發 |
| _V2.7.2 原文_ | (1) **修返回手勢關閉 App**：PWA 單頁無歷史，Android 邊緣返回手勢直接退出。以 History API 接管——render 後 pushState 當前 screen，`popstate` 攔截退一層（quiz/result→該層選單、子選單→上層、首頁為根才放行真正離開），用 `window.history` 相容 jsdom；(2) **#1 地圖防護網**：validate.py 納入 `world.js`——196 國輪廓齊全、必要欄位、path 以 M 開頭、bb 範圍與順序、cx/cy 落在 bb 內、洲別白名單、contBox 六洲、名稱與題庫一致（故障注入驗證三種錯誤均攔下）。常駐測試加返回手勢段 |
| _V2.7.1 原文_ | **全庫語句盤檢**：validator 機械層全綠（過短/亂碼/重複/洩答零問題，證明 V2.5.0 後新資料都被守過）；啟發式撈出 124 對疑似重複，人工複核挑出 **11 條真正同義反覆**改寫分散（直接改 data-src 主檔，delta 只增不改）；validator 補中文語法註解，記錄「缺動詞≠半截句」避免再生 2847 條誤報 |
| _V2.7.0 原文_ | **首都冷知識**：新資料集 `data-src/capital-facts.json`（196 首都 ×3＝588 條，key=國名）；build 產 `CAP_FACTS`（**key 轉為首都名**，對齊首都題的 S.target）；首都題答完**優先顯示首都冷知識**（無則退回國家小知識）；validate 加覆蓋率＋交叉比對；常駐測試斷言改首都名。資料集從 8 增為 9 |
| _V2.6.0 原文_ | **熱門國地標擴充**：32 個熱門國各 +3 個世界地標＋每地標 3 句說明（共 +96 地標、+288 句），首次以 delta 增量流落地（兩個 deltas/ 檔 build 時合併歸檔，未動主檔其餘部分）。世界地標庫 183→279、地標說明 549→837。北韓由 0 補到 3 |
| _V2.5.0 原文_ | **內容基礎建設**：`data-src/` 8 個 JSON 單一真相（schema_version=1、條目帶 angle 標籤）→ `tools/build.py`（合併 deltas 只增不改、剝標籤產 GENERATED js、CAPMAP 派生、manifest 對帳）→ `tools/validate.py` 內容 CI（覆蓋率下限／P7 洩答全套／重名／句子品質 6–40 字禁亂碼／角度白名單／交叉比對／**往返一致性防雙檔漂移**）→ `tools/bump.py` 一鍵升版 → `tests/run-tests.js` 常駐 jsdom（內建導航地圖）→ `make.sh` 一條龍。首跑即抓 39 個真問題（38 條 4–5 字斷句＋1 條說明重複）全修。舊 `data/landmarks.json` 移除；零依賴（純 Python 標準庫）；app 執行期零改動 |
