...
| V2.7.2 | (1) **修返回手勢關閉 App**：PWA 單頁無歷史，Android 邊緣返回手勢直接退出。以 History API 接管——render 後 pushState 當前 screen，`popstate` 攔截退一層（quiz/result→該層選單、子選單→上層、首頁為根才放行真正離開），用 `window.history` 相容 jsdom；(2) **#1 地圖防護網**：validate.py 納入 `world.js`——196 國輪廓齊全、必要欄位、path 以 M 開頭、bb 範圍與順序、cx/cy 落在 bb 內、洲別白名單、contBox 六洲、名稱與題庫一致（故障注入驗證三種錯誤均攔下）。常駐測試加返回手勢段 |
