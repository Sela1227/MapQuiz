...
| V1.4.0 | (1) **滾輪縮放**：#app 上委派 wheel（passive:false、一格 ±0.5）；世界題 zoom 下限 0.2——`S.zoom<1` 時不縮 SVG 而是 `expandIfZoomedOut()` 把 focus viewBox 以中心放大（夾在全圖內），達成「縮小看更大範圍」；台灣／非聚焦下限仍 1；(2) **選洲作答**：世界選單加 `contchip` 列（全部＋六洲，`S.worldCont`），題池/地標池過濾該洲、選單預覽地圖聚焦該洲；(3) **世界地標 138→183**（+45：巴爾幹/中亞/西非/加勒比/大洋洲；新增 35 國覆蓋，圭亞那等名稱以 world_names.txt 為準）|
...
V1.4.0：滾輪快速縮放（世界題可縮小到近全圖——viewBox 擴張而非縮 SVG）、世界選洲（chips 過濾題池與預覽）、地標擴至 183（125 國）。jsdom 全綠。下版優先：SELA 實玩回饋。
